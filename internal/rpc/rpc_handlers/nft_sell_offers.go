...
func (m *NftSellOffersMethod) Handle(ctx *rpc_types.RpcContext, params json.RawMessage) (interface{}, *rpc_types.RpcError) {
	...
	result, err := rpc_types.Services.Ledger.GetNFTSellOffers(nftID, ledgerIndex, limit, marker)
	if err != nil {
		...
		return nil, rpc_types.RpcErrorInternal("Failed to get NFT sell offers: " + err.Error())
	}
