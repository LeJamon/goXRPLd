...
func TestEncodeSeed(t *testing.T) {
	tt := []struct {
		...
		},
		{
			...
			input:          []byte("testingsomething"),
			cryptoType:     crypto.CryptoType(999), // Using an invalid crypto type
			expectedOutput: "",
			expectedErr:    errors.New("encoding type must be `ed25519` or `secp256k1`"),
		},
		{
			name:           "fail - invalid CryptoAlgorithm type returns err",
			input:          []byte("testingsomething"),
			cryptoType:     crypto.CryptoType(999), // Using an invalid crypto type
			expectedOutput: "",
			expectedErr:    errors.New("encoding type must be `ed25519` or `secp256k1`"),
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			got, err := EncodeSeed(tc.input, tc.cryptoType)
			if tc.expectedErr != nil {
				require.EqualError(t, err, tc.expectedErr.Error())
			} else {
				require.Equal(t, tc.expectedOutput, got)
