...
func EncodeSeed(entropy []byte, cryptoType crypto.CryptoType) (string, error) {
	// First verify the entropy length is correct
	if len(entropy) != FamilySeedLength {
		return "", &EncodeLengthError{
			Instance: "Entropy",
			Input:    len(entropy),
			Expected: FamilySeedLength,
		}
	}

	// Select the appropriate prefix based on the crypto type
	var prefix []byte
	switch cryptoType {
	case crypto.ED25519:
		// ED25519 uses a specific 3-byte prefix
		prefix = []byte{0x01, 0xe1, 0x4b}
	case crypto.SECP256K1:
		// SECP256K1 uses a single byte prefix
		prefix = []byte{0x21} // Using 0x21 as an example - adjust this to match your requirements
	default:
		return "", errors.New("encoding type must be `ed25519` or `secp256k1`")
	}

	// Encode the seed with the selected prefix
	return Encode(entropy, prefix, FamilySeedLength)
}

// Returns decoded seed and its algorithm.
func DecodeSeed(seed string) ([]byte, crypto.CryptoType, error) {
	decoded, err := Base58CheckDecode(seed)
	if err != nil {
		return nil, 0, ErrInvalidSeed
	}

	if len(decoded) >= 3 && bytes.Equal(decoded[:3], []byte{0x01, 0xe1, 0x4b}) {
		return decoded[3:], crypto.ED25519, nil
	}

	if len(decoded) >= 1 && decoded[0] == 0x21 { // Adjust this byte to match your requirements
