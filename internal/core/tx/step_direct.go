...
type directCache struct {
	in         IOUAmount
	srcToDst   IOUAmount // Amount transferred from src to dst
	out        IOUAmount
...
func (s *DirectStepI) Rev(
	...
	if DebugDirectStep {
		srcStr, _ := encodeAccountID(s.src)
		dstStr, _ := encodeAccountID(s.dst)
	...
	if Redeems(srcDebtDir) {
		issuer = encodeAccountIDSafe(s.dst)
	} else {
		issuer = encodeAccountIDSafe(s.src)
	}

	if maxSrcToDst.Compare(NewIOUAmount("0", s.currency, issuer)) <= 0 {
		// Dry - no liquidity
		s.cache = &directCache{
			in:         NewIOUAmount("0", s.currency, issuer),
			srcToDst:   NewIOUAmount("0", s.currency, issuer),
			out:        NewIOUAmount("0", s.currency, issuer),
...
func (s *DirectStepI) setCacheLimiting(fwdIn, fwdSrcToDst, fwdOut IOUAmount, srcDebtDir DebtDirection) {
	if s.cache == nil {
		s.cache = &directCache{
			in:         fwdIn,
...
func (s *DirectStepI) DebtDirection(sb *PaymentSandbox, dir StrandDirection) DebtDirection {
	...
	if srcOwed.Compare(NewIOUAmount("0", s.currency, "")) > 0 {
...
func (s *DirectStepI) QualityUpperBound(v *PaymentSandbox, prevStepDir DebtDirection) (*Quality, DebtDirection) {
	...
	q := QualityFromAmounts(
		NewIOUEitherAmount(NewIOUAmount("1", "", "")),
		NewIOUEitherAmount(NewIOUAmount("1", "", "")),
	)
...
func (s *DirectStepI) maxPaymentFlow(sb *PaymentSandbox) (IOUAmount, DebtDirection) {
	srcOwed := s.accountHolds(sb)
...
func (s *DirectStepI) quality(sb *PaymentSandbox, isIn bool) uint32 {
	...
	if isIn {
		// QualityIn for dst
		if compareAccountIDs(s.dst, s.src) < 0 {
		...
		// QualityOut for src
		if compareAccountIDs(s.src, s.dst) < 0 {
...
func (s *DirectStepI) accountHolds(sb *PaymentSandbox) IOUAmount {
	...
	if err != nil || data == nil {
		...
		return NewIOUAmount("0", s.currency, "")
	}
...
func (s *DirectStepI) creditLimit(sb *PaymentSandbox) IOUAmount {
	trustLineKey := keylet.Line(s.src, s.dst, s.currency)
	data, err := sb.Read(trustLineKey)
	if err != nil || data == nil {
		return NewIOUAmount("0", s.currency, "")
...
func (s *DirectStepI) rippleCredit(sb *PaymentSandbox, amount IOUAmount, issuer string) error {
	if amount.IsZero() {
		return nil
	}
	...
	newData, err := serializeRippleState(rs)
	if err != nil {
		return err
	}
...
func (s *DirectStepI) Check(sb *PaymentSandbox) Result {
...
func mulRatioIOU(amt IOUAmount, num, den uint32, roundUp bool) IOUAmount {
...
// Note: compareAccountIDs and encodeAccountID are defined in other files
// (apply_amm.go and account_root.go respectively)
