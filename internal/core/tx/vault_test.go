...
func TestVaultCreateValidation(t *testing.T) {
	tests := []struct {
		...
		// Valid cases
		{
			...
			tx:      NewVaultCreate("rOwner", Asset{Currency: "XRP"}),
			wantErr: false,
		},
		{
			name: "valid - vault create with IOU",
			tx:   NewVaultCreate("rOwner", Asset{Currency: "USD", Issuer: "rIssuer"}),
			wantErr: false,
		},
		{
			name: "valid - vault create with Data",
			tx: func() *VaultCreate {
				v := NewVaultCreate("rOwner", Asset{Currency: "XRP"})
...
func TestVaultDepositValidation(t *testing.T) {
	tests := []struct {
		...
		// Valid cases
		{
			...
			tx:      NewVaultDeposit("rOwner", makeValidVaultID(), NewXRPAmount("1000000")),
			wantErr: false,
		},
		{
			name:    "valid - deposit IOU",
			tx:      NewVaultDeposit("rOwner", makeValidVaultID(), NewIssuedAmount("100", "USD", "rIssuer")),
			wantErr: false,
		},
		...
		{
			name:    "invalid - missing VaultID",
			tx:      NewVaultDeposit("rOwner", "", NewXRPAmount("1000000")),
			wantErr: true,
			errMsg:  "VaultID is required",
		},
		{
			name:    "invalid - VaultID is zero",
			tx:      NewVaultDeposit("rOwner", makeZeroVaultID(), NewXRPAmount("1000000")),
			wantErr: true,
			errMsg:  "VaultID cannot be zero",
		},
		{
			name:    "invalid - missing Amount",
			tx:      NewVaultDeposit("rOwner", makeValidVaultID(), Amount{}),
			wantErr: true,
			errMsg:  "Amount is required",
		},
		{
			name:    "invalid - zero Amount",
			tx:      NewVaultDeposit("rOwner", makeValidVaultID(), NewXRPAmount("0")),
			wantErr: true,
			errMsg:  "positive",
		},
		{
			name:    "invalid - negative Amount",
			tx:      NewVaultDeposit("rOwner", makeValidVaultID(), NewXRPAmount("-100")),
			wantErr: true,
			errMsg:  "positive",
		},
		{
			...
			tx: func() *VaultDeposit {
				v := NewVaultDeposit("rOwner", makeValidVaultID(), NewXRPAmount("1000000"))
				flags := uint32(tfUniversal)
				v.Common.Flags = &flags
				return v
			}(),
...
func TestVaultWithdrawValidation(t *testing.T) {
	tests := []struct {
		...
		// Valid cases
		{
			...
			tx:      NewVaultWithdraw("rOwner", makeValidVaultID(), NewXRPAmount("1000000")),
			wantErr: false,
		},
		{
			name:    "valid - withdraw IOU",
			tx:      NewVaultWithdraw("rOwner", makeValidVaultID(), NewIssuedAmount("100", "USD", "rIssuer")),
			wantErr: false,
		},
		{
			name: "valid - withdraw with Destination",
			tx: func() *VaultWithdraw {
				v := NewVaultWithdraw("rOwner", makeValidVaultID(), NewXRPAmount("1000000"))
		...
		{
			name:    "invalid - missing VaultID",
			tx:      NewVaultWithdraw("rOwner", "", NewXRPAmount("1000000")),
			wantErr: true,
			errMsg:  "VaultID is required",
		},
		{
			name:    "invalid - VaultID is zero",
			tx:      NewVaultWithdraw("rOwner", makeZeroVaultID(), NewXRPAmount("1000000")),
			wantErr: true,
			errMsg:  "VaultID cannot be zero",
		},
		{
			name:    "invalid - missing Amount",
			tx:      NewVaultWithdraw("rOwner", makeValidVaultID(), Amount{}),
			wantErr: true,
			errMsg:  "Amount is required",
		},
		{
			name:    "invalid - zero Amount",
			tx:      NewVaultWithdraw("rOwner", makeValidVaultID(), NewXRPAmount("0")),
			wantErr: true,
			errMsg:  "positive",
		},
		{
			name:    "invalid - negative Amount",
			tx:      NewVaultWithdraw("rOwner", makeValidVaultID(), NewXRPAmount("-100")),
			wantErr: true,
			errMsg:  "positive",
		},
		{
...
func TestVaultClawbackValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tx: func() *VaultClawback {
				v := NewVaultClawback("rIssuer", makeValidVaultID(), "rHolder")
				amt := NewIssuedAmount("0", "USD", "rIssuer")
				v.Amount = &amt
				return v
			}(),
			wantErr: false,
		...
		},
		{
			...
			tx: func() *VaultClawback {
				v := NewVaultClawback("rIssuer", makeValidVaultID(), "rHolder")
				amt := NewIssuedAmount("100", "USD", "rOtherIssuer") // Different issuer
				v.Amount = &amt
				return v
			}(),
			wantErr: true,
...
func TestVaultConstructors(t *testing.T) {
	t.Run("NewVaultCreate", func(t *testing.T) {
		...
		assert.Equal(t, TypeVaultCreate, tx.TxType())
	})
	...
	t.Run("NewVaultSet", func(t *testing.T) {
		...
		assert.Equal(t, TypeVaultSet, tx.TxType())
	})
	...
	t.Run("NewVaultDelete", func(t *testing.T) {
		...
		assert.Equal(t, "rOwner", tx.Account)
		assert.Equal(t, makeValidVaultID(), tx.VaultID)
		assert.Equal(t, TypeVaultDelete, tx.TxType())
	})
	...
	t.Run("NewVaultDeposit", func(t *testing.T) {
		...
		assert.Equal(t, TypeVaultDeposit, tx.TxType())
	})
	...
	t.Run("NewVaultWithdraw", func(t *testing.T) {
		...
		assert.Equal(t, makeValidVaultID(), tx.VaultID)
		assert.Equal(t, "1000000", tx.Amount.Value)
		assert.Equal(t, TypeVaultWithdraw, tx.TxType())
	})
	...
	t.Run("NewVaultClawback", func(t *testing.T) {
		...
		assert.Equal(t, TypeVaultClawback, tx.TxType())
	})
}
