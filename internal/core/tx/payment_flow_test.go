...
func (m *paymentMockLedgerView) createAccount(accountID [20]byte, balanceDrops uint64, ownerCount uint32) {
	...
	data, _ := serializeAccountRoot(account)
...
func (m *paymentMockLedgerView) createTrustLine(low, high [20]byte, currency string, balanceLow int64, limitLow, limitHigh int64) {
	...
	rs := &RippleState{
		Balance:   NewIOUAmount(formatInt64(balanceLow), currency, highIssuer),
		LowLimit:  NewIOUAmount(formatInt64(limitLow), currency, lowIssuer),
		HighLimit: NewIOUAmount(formatInt64(limitHigh), currency, highIssuer),
		LowQualityIn:   QualityOne,
		LowQualityOut:  QualityOne,
		HighQualityIn:  QualityOne,
		HighQualityOut: QualityOne,
	}
	data, _ := serializeRippleState(rs)
...
func TestEitherAmount_IOU(t *testing.T) {
	...
	iou := NewIOUAmount("100", "USD", "rIssuer123")
...
func TestEitherAmount_Add(t *testing.T) {
	...
	// Test IOU addition
	iouA := NewIOUEitherAmount(NewIOUAmount("100", "USD", "issuer"))
	iouB := NewIOUEitherAmount(NewIOUAmount("50", "USD", "issuer"))
	iouC := iouA.Add(iouB)
...
func TestPaymentSandbox_Isolation(t *testing.T) {
	...
	originalAccount, _ := parseAccountRoot(originalData)
...
func TestPaymentSandbox_ChildSandbox(t *testing.T) {
	...
	childAccount, _ := parseAccountRoot(childData)
	...
	parentAccount, _ := parseAccountRoot(parentData)
	...
	parentAccount2, _ := parseAccountRoot(parentData2)
...
func TestDirectStepI_Basic(t *testing.T) {
	...
	if result != TesSUCCESS {
		t.Errorf("expected TesSUCCESS, got %d", result)
	}
}
...
func TestToStrands_WithPaths(t *testing.T) {
	...
	dstAmt := NewIssuedAmount("100", "USD", encodeAccountIDSafe(gateway))
	paths := [][]PathStep{
		{{Currency: "USD", Issuer: encodeAccountIDSafe(gateway)}},
	}
...
func TestFlow_SingleStrand(t *testing.T) {
	...
	if result.Result != TesSUCCESS {
		t.Errorf("expected TesSUCCESS, got %d", result.Result)
	}
	if result.Out.XRP != 10_000_000 {
		t.Errorf("expected output=10M, got %d", result.Out.XRP)
	}
...
func TestFlow_PartialPayment(t *testing.T) {
	...
	if result2.Out.XRP == 0 && result2.Result != TecPATH_DRY {
...
func TestFlow_EmptyStrands(t *testing.T) {
	...
	if result.Result != TecPATH_DRY {
		t.Errorf("expected TecPATH_DRY for empty strands, got %d", result.Result)
	}
}
...
func TestRippleCalculate_XRPPayment(t *testing.T) {
	...
	dstAmount := NewXRPAmount("10000000") // 10 XRP
	var txHash [32]byte
	...
	if result != TesSUCCESS && result != TecPATH_DRY {
		t.Errorf("expected TesSUCCESS or TecPATH_DRY, got %d", result)
	}
...
func TestMulRatio_IOU(t *testing.T) {
	iou := NewIOUAmount("100", "USD", "issuer")
	amt := NewIOUEitherAmount(iou)
