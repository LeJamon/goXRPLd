...
// These tests are translated from rippled's Check_test.cpp focusing on
// validation logic and error conditions from testCreateValid and testCreateInvalid.
func TestCheckCreateValidation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			check: &CheckCreate{
				BaseTx:      *NewBaseTx(TypeCheckCreate, "rAlice"),
				Destination: "rBob",
				SendMax:     NewXRPAmount("10000000000"), // 10000 XRP in drops
			},
			expectError: false,
		},
		{
			...
			check: &CheckCreate{
				BaseTx:      *NewBaseTx(TypeCheckCreate, "rAlice"),
				Destination: "rBob",
				SendMax:     NewIssuedAmount("50", "USD", "rGateway"),
			},
			expectError: false,
		},
		{
			...
			check: &CheckCreate{
				BaseTx:         *NewBaseTx(TypeCheckCreate, "rAlice"),
				Destination:    "rBob",
				SendMax:        NewXRPAmount("10000000000"),
		...
		},
		{
			...
			check: &CheckCreate{
				BaseTx:      BaseTx{Common: Common{TransactionType: "CheckCreate"}},
				Destination: "rBob",
				SendMax:     NewXRPAmount("10000000000"),
			},
			expectError: true,
...
// Tests translated from rippled's testCashXRP and testCashIOU.
func TestCheckCashValidation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			check: &CheckCash{
				BaseTx:  *NewBaseTx(TypeCheckCash, "rBob"),
				CheckID: "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
				Amount:  &Amount{Value: "10000000000"},
			},
			expectError: false,
		},
		{
			...
			check: &CheckCash{
				BaseTx:     *NewBaseTx(TypeCheckCash, "rBob"),
				CheckID:    "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
				DeliverMin: &Amount{Value: "5000000000"},
			},
			expectError: false,
		},
		{
			...
			check: &CheckCash{
				BaseTx:  *NewBaseTx(TypeCheckCash, "rBob"),
				CheckID: "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
				Amount:  ptrAmount(NewIssuedAmount("10", "USD", "rGateway")),
			},
			expectError: false,
		},
		{
			...
			check: &CheckCash{
				BaseTx:     *NewBaseTx(TypeCheckCash, "rBob"),
				CheckID:    "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
				DeliverMin: ptrAmount(NewIssuedAmount("5", "USD", "rGateway")),
			},
			expectError: false,
		},
		{
		...
		},
		{
			name: "missing account",
			check: &CheckCash{
				BaseTx:  BaseTx{Common: Common{TransactionType: "CheckCash"}},
				CheckID: "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
				Amount:  &Amount{Value: "10000000000"},
			},
			expectError: true,
...
func TestCheckCancelValidation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "valid cancel",
			check: &CheckCancel{
				BaseTx:  *NewBaseTx(TypeCheckCancel, "rBob"),
				CheckID: "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
			},
			expectError: false,
		},
		{
			...
			check: &CheckCancel{
				BaseTx:  *NewBaseTx(TypeCheckCancel, "rAlice"),
				CheckID: "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
			},
			expectError: false,
		},
		...
		},
		{
			name: "missing account",
			check: &CheckCancel{
				BaseTx:  BaseTx{Common: Common{TransactionType: "CheckCancel"}},
				CheckID: "49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0",
			},
			expectError: true,
			errorMsg:    "Account is required",
...
func TestCheckTransactionTypes(t *testing.T) {
	t.Run("CheckCreate type", func(t *testing.T) {
		c := NewCheckCreate("rAlice", "rBob", NewXRPAmount("1000000"))
		if c.TxType() != TypeCheckCreate {
...
func TestCheckCashHelperMethods(t *testing.T) {
	...
	t.Run("SetExactAmount clears DeliverMin", func(t *testing.T) {
		c := NewCheckCash("rBob", "DEADBEEF")
		c.SetDeliverMin(NewXRPAmount("500000"))
		c.SetExactAmount(NewXRPAmount("1000000"))
...
// Helper function to create a pointer to Amount
