...
func (p *Payment) Apply(ctx *ApplyContext) Result {
...
func (p *Payment) applyXRPPayment(ctx *ApplyContext) Result {
	// Parse the amount
	...
	destAccountID, err := decodeAccountID(p.Destination)
	if err != nil {
	...
	if destExists {
		...
		if (destAccount.Flags & lsfDisallowXRP) != 0 {
			senderAccountID, err := decodeAccountID(ctx.Account.Account)
			if err != nil {
				return TefINTERNAL
			}
		...
		if (destAccount.Flags&0x00020000) != 0 && p.DestinationTag == nil {
...
func (p *Payment) applyIOUPayment(ctx *ApplyContext) Result {
	// Parse the amount
	amount := NewIOUAmount(p.Amount.Value, p.Amount.Currency, p.Amount.Issuer)
	if amount.IsZero() {
		return TemBAD_AMOUNT
	}
	...
	issuerAccountID, err := decodeAccountID(p.Amount.Issuer)
	if err != nil {
	...
	if result == TesSUCCESS && p.DeliverMin != nil {
		flags := p.GetFlags()
		if (flags & PaymentFlagPartialPayment) != 0 {
			deliverMin := NewIOUAmount(p.DeliverMin.Value, p.DeliverMin.Currency, p.DeliverMin.Issuer)
...
func (p *Payment) applyIOUPaymentWithPaths(ctx *ApplyContext, senderID, destID, issuerID [20]byte) Result {
...
func (p *Payment) applyIOUIssue(ctx *ApplyContext, dest *AccountRoot, senderID, destID [20]byte, amount IOUAmount) Result {
...
func (p *Payment) applyIOURedeem(ctx *ApplyContext, dest *AccountRoot, senderID, destID [20]byte, amount IOUAmount) Result {
...
func (p *Payment) applyIOUTransfer(ctx *ApplyContext, dest *AccountRoot, senderID, destID, issuerID [20]byte, amount IOUAmount) Result {
...
func (p *Payment) applyIOUIssueWithDelivered(ctx *ApplyContext, dest *AccountRoot, senderID, destID [20]byte, amount IOUAmount) (Result, IOUAmount) {
...
func (p *Payment) applyIOURedeemWithDelivered(ctx *ApplyContext, dest *AccountRoot, senderID, destID [20]byte, amount IOUAmount) (Result, IOUAmount) {
...
func (p *Payment) applyIOUTransferWithDelivered(ctx *ApplyContext, dest *AccountRoot, senderID, destID, issuerID [20]byte, amount IOUAmount) (Result, IOUAmount) {
