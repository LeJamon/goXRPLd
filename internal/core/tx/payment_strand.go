...
func ToStrands(
	...
	if DebugStrands {
		srcStr, _ := encodeAccountID(src)
		dstStr, _ := encodeAccountID(dst)
...
func ToStrand(
	...
	for _, elem := range path {
		...
		if hasIssuer(elem) {
			issuerBytes, err := decodeAccountID(elem.Issuer)
			if err == nil {
	...
	if DebugStrands {
		...
		for i, n := range normPath {
			acctStr := "none"
			if n.hasAccount {
				acctStr, _ = encodeAccountID(n.account)
			}
			issuerStr := "none"
			if n.hasIssuer {
				issuerStr, _ = encodeAccountID(n.issuer)
			}
	...
	if DebugStrands {
		...
		for i, s := range strand {
			if accts := s.DirectStepAccts(); accts != nil {
				srcStr, _ := encodeAccountID(accts[0])
				dstStr, _ := encodeAccountID(accts[1])
				fmt.Printf("  Step %d: DirectStep %s -> %s\n", i, srcStr, dstStr)
			} else if book := s.BookStepBook(); book != nil {
				fmt.Printf("  Step %d: BookStep %s/%s -> %s/%s\n", i, book.In.Currency, encodeAccountIDSafe(book.In.Issuer), book.Out.Currency, encodeAccountIDSafe(book.Out.Issuer))
			} else {
...
func accountFromPathElement(elem PathStep, defaultAccount [20]byte) [20]byte {
	if elem.Account != "" {
		accountBytes, err := decodeAccountID(elem.Account)
		if err == nil {
			return accountBytes
		}
	}
...
func CheckStrand(strand Strand, view *PaymentSandbox) Result {
	for _, step := range strand {
		// Check if step implements Check method
		if checker, ok := step.(interface{ Check(*PaymentSandbox) Result }); ok {
