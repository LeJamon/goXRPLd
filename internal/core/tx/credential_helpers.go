...
func parseCredentialEntry(data []byte) (*CredentialEntry, error) {
	...
	if subject, ok := jsonObj["Subject"].(string); ok {
		subjectID, err := decodeAccountID(subject)
		if err == nil {
	...
	if issuerNode, ok := jsonObj["IssuerNode"].(string); ok {
		cred.IssuerNode = parseUint64Hex(issuerNode)
	}

	// Parse SubjectNode
	if subjectNode, ok := jsonObj["SubjectNode"].(string); ok {
		cred.SubjectNode = parseUint64Hex(subjectNode)
	}
...
func serializeCredentialEntry(cred *CredentialEntry) ([]byte, error) {
	...
	// Add Subject
	subjectStr, err := encodeAccountID(cred.Subject)
	if err == nil && subjectStr != "" {
		jsonObj["Subject"] = subjectStr
	}

	// Add Issuer
	issuerStr, err := encodeAccountID(cred.Issuer)
	...
	// Add IssuerNode
	jsonObj["IssuerNode"] = formatUint64Hex(cred.IssuerNode)

	// Add SubjectNode (if subject != issuer)
	if cred.Subject != cred.Issuer {
		jsonObj["SubjectNode"] = formatUint64Hex(cred.SubjectNode)
	}
