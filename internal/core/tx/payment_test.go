	...
	tests := []struct {
		...
		// Valid payment cases
		{
			...
			payment: &Payment{
				...
				Amount:      NewXRPAmount("1000000"), // 1 XRP in drops
				Destination: "rBob",
			},
			expectError: false,
		},
		...
		},
		{
			...
			payment: &Payment{
				BaseTx:      BaseTx{Common: Common{TransactionType: "Payment"}},
				Amount:      NewXRPAmount("1000000"),
				Destination: "rBob",
			},
			expectError: true,
...
func TestPaymentAmounts(t *testing.T) {
	tests := []struct {
		...
		// XRP amounts
		{
			...
			payment: &Payment{
				BaseTx:      *NewBaseTx(TypePayment, "rAlice"),
				Amount:      NewXRPAmount("1000000"), // 1 XRP
				Destination: "rBob",
			},
			expectError: false,
		},
		{
			...
			payment: &Payment{
				BaseTx:      *NewBaseTx(TypePayment, "rAlice"),
				Amount:      NewXRPAmount("100000000000000"), // 100M XRP
				Destination: "rBob",
			},
			expectError: false,
		},
		...
		},
		{
			...
			payment: &Payment{
				BaseTx:      *NewBaseTx(TypePayment, "rAlice"),
				Amount:      NewIssuedAmount("100.50", "USD", "rGateway"),
				Destination: "rBob",
			},
			expectError: false,
		},
		{
			...
			payment: &Payment{
				BaseTx:      *NewBaseTx(TypePayment, "rAlice"),
				Amount:      NewIssuedAmount("1e10", "USD", "rGateway"),
				Destination: "rBob",
			},
			expectError: false,
		},
...
func TestPaymentFlags(t *testing.T) {
	t.Run("tfPartialPayment flag", func(t *testing.T) {
		payment := NewPayment("rAlice", "rBob", NewXRPAmount("1000000"))
	...
	t.Run("tfNoDirectRipple flag", func(t *testing.T) {
		payment := NewPayment("rAlice", "rBob", NewIssuedAmount("100", "USD", "rGateway"))
	...
	t.Run("partial payment with DeliverMin", func(t *testing.T) {
		payment := NewPayment("rAlice", "rBob", NewXRPAmount("1000000"))
		payment.SetPartialPayment()
		deliverMin := NewXRPAmount("500000")
	...
	tests := []struct {
		...
		},
		{
			...
			payment: &Payment{
				...
				Amount:      NewIssuedAmount("100", "EUR", "rGatewayEUR"),
				Destination: "rBob",
				SendMax:     ptrAmount(NewIssuedAmount("120", "USD", "rGatewayUSD")),
			},
			checkMap: func(t *testing.T, m map[string]any) {
	...
	}{
		{
			...
			payment: func() *Payment {
				p := &Payment{
					...
					Amount:      NewXRPAmount("1000000"),
					Destination: "rBob",
					DeliverMin:  ptrAmount(NewXRPAmount("500000")),
				}
				p.SetPartialPayment()
				return p
			}(),
		...
		},
		{
			...
			payment: func() *Payment {
				p := &Payment{
					...
					Amount:      NewIssuedAmount("100", "USD", "rGateway"),
					Destination: "rBob",
					DeliverMin:  ptrAmount(NewIssuedAmount("50", "USD", "rGateway")),
				}
				p.SetPartialPayment()
				return p
			}(),
...
func TestPaymentPaths(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			payment: &Payment{
				BaseTx:      *NewBaseTx(TypePayment, "rAlice"),
				Amount:      NewIssuedAmount("100", "JPY", "rGatewayJPY"),
				Destination: "rBob",
				SendMax:     ptrAmount(NewIssuedAmount("150", "USD", "rGatewayUSD")),
				Paths: [][]PathStep{
					{
						{Currency: "EUR", Issuer: "rGatewayEUR"},
						{Currency: "XRP"},
	...
	tests := []struct {
		...
		},
		{
			...
			payment: &Payment{
				...
				InvoiceID:      "DEADBEEF",
				SendMax:        ptrAmount(NewXRPAmount("1100000")),
				DeliverMin:     ptrAmount(NewXRPAmount("900000")),
				Paths: [][]PathStep{
					{
						{Currency: "XRP"},
					},
...
func TestPaymentTransactionType(t *testing.T) {
	payment := NewPayment("rAlice", "rBob", NewXRPAmount("1000000"))
	if payment.TxType() != TypePayment {
		t.Errorf("expected TypePayment, got %v", payment.TxType())
	}
}
...
func TestPaymentZeroAmount(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			payment: &Payment{
				BaseTx:      *NewBaseTx(TypePayment, "rAlice"),
				Amount:      NewIssuedAmount("0", "USD", "rGateway"),
				Destination: "rBob",
			},
			expectError: false,
		},
...
func TestAmountIsNative(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			amount:   Amount{Value: "1000000", Currency: "", Issuer: ""},
			expected: true,
		},
	}
...
func TestPaymentCrossCurrency(t *testing.T) {
	...
	t.Run("XRP to IOU cross-currency", func(t *testing.T) {
		payment := &Payment{
			...
			Amount:      NewIssuedAmount("100", "USD", "rGateway"),
			Destination: "rBob",
			SendMax:     ptrAmount(NewXRPAmount("110000000")), // 110 XRP in drops
			Paths: [][]PathStep{
				{
