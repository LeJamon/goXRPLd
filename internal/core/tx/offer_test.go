	...
	tests := []struct {
		...
		// Valid offer scenarios
		{
			...
			offer: &OfferCreate{
				...
				TakerGets: NewXRPAmount("1000000000"),                      // 1000 XRP in drops
				TakerPays: NewIssuedAmount("100", "USD", "rGateway"),       // 100 USD
			},
			expectError: false,
		},
		{
			...
			offer: &OfferCreate{
				BaseTx:    *NewBaseTx(TypeOfferCreate, "rAlice"),
				TakerGets: NewIssuedAmount("100", "USD", "rGateway"),       // 100 USD
				TakerPays: NewXRPAmount("1000000000"),                      // 1000 XRP in drops
			},
			expectError: false,
		},
		{
			...
			offer: &OfferCreate{
				BaseTx:    *NewBaseTx(TypeOfferCreate, "rAlice"),
				TakerGets: NewIssuedAmount("100", "USD", "rGateway"),       // 100 USD
				TakerPays: NewIssuedAmount("80", "EUR", "rEuroGateway"),    // 80 EUR
			},
			expectError: false,
		},
		...
		},
		{
			...
			offer: &OfferCreate{
				BaseTx:    *NewBaseTx(TypeOfferCreate, "rAlice"),
				TakerGets: NewXRPAmount("1000000000"),
		...
		},
		{
			...
			offer: &OfferCreate{
				BaseTx:    BaseTx{Common: Common{TransactionType: "OfferCreate"}},
				TakerGets: NewXRPAmount("1000000000"),
				TakerPays: NewIssuedAmount("100", "USD", "rGateway"),
			},
			expectError: true,
...
func TestOfferCancelValidation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "valid cancel",
			offer: &OfferCancel{
				BaseTx:        *NewBaseTx(TypeOfferCancel, "rAlice"),
				OfferSequence: 12345,
			},
			expectError: false,
		},
		...
		},
		{
			name: "missing account",
			offer: &OfferCancel{
				BaseTx:        BaseTx{Common: Common{TransactionType: "OfferCancel"}},
				OfferSequence: 12345,
			},
			expectError: true,
			errorMsg:    "Account is required",
	...
	tests := []struct {
		...
		},
		{
			...
			offer: &OfferCreate{
				...
				TakerPays: NewXRPAmount("1000000000"),
			},
			checkMap: func(t *testing.T, m map[string]any) {
		...
		},
		{
			...
			offer: &OfferCreate{
				BaseTx:    *NewBaseTx(TypeOfferCreate, "rAlice"),
				TakerGets: NewIssuedAmount("100", "USD", "rGateway"),
				TakerPays: NewIssuedAmount("80", "EUR", "rEuroGateway"),
			},
			checkMap: func(t *testing.T, m map[string]any) {
				takerGets, ok := m["TakerGets"].(map[string]any)
				if !ok {
...
func TestOfferTransactionTypes(t *testing.T) {
	t.Run("OfferCreate type", func(t *testing.T) {
		o := NewOfferCreate("rAlice", NewXRPAmount("1000000"), NewIssuedAmount("10", "USD", "rGateway"))
		if o.TxType() != TypeOfferCreate {
			t.Errorf("expected TypeOfferCreate, got %v", o.TxType())
		}
	})

	t.Run("OfferCancel type", func(t *testing.T) {
		o := NewOfferCancel("rAlice", 123)
		if o.TxType() != TypeOfferCancel {
			t.Errorf("expected TypeOfferCancel, got %v", o.TxType())
		}
	})
}
...
func TestNewOfferConstructors(t *testing.T) {
	t.Run("NewOfferCreate", func(t *testing.T) {
		...
		takerPays := NewIssuedAmount("10", "USD", "rGateway")
...
func TestOfferCreateAmountTypes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			takerGets:   NewIssuedAmount("100", "USD", "rGateway"),
			takerPays:   NewIssuedAmount("90", "EUR", "rEuroGateway"),
			expectError: false,
		},
		{
			name:        "same IOU both sides - different issuers is valid",
			takerGets:   NewIssuedAmount("100", "USD", "rGateway1"),
			takerPays:   NewIssuedAmount("100", "USD", "rGateway2"),
			expectError: false,
		},
		{
		...
		},
		{
			...
			takerGets:   NewXRPAmount("100000000000000"), // 100 million XRP
			takerPays:   NewIssuedAmount("1000000", "USD", "rGateway"),
			expectError: false,
		},
		{
			name:        "small XRP amount (1 drop)",
			takerGets:   NewXRPAmount("1"),
			takerPays:   NewIssuedAmount("0.000001", "USD", "rGateway"),
			expectError: false,
		},
	}
