...
func RippleCalculate(
	...
	txHash [32]byte,
	ledgerSeq uint32,
) (EitherAmount, EitherAmount, map[[32]byte]bool, *PaymentSandbox, Result) {
	...
	if err != nil || len(strands) == 0 {
		return ZeroXRPEitherAmount(), ZeroXRPEitherAmount(), nil, nil, TecPATH_DRY
	}
	...
	if result.Result == TesSUCCESS || result.Result == TecPATH_PARTIAL {
