...
func TestPromotionColdWritableReadDoesNotBlockPut(t *testing.T) {
	...
	store := newPromotionReadStore(t, block, []byte("key"), []byte("old"), 1<<20)
	...
	waitPromotionSignal(t, block.entered, "cold writable read")
	put := make(chan error, 1)
	go func() { put <- store.Put([]byte("key"), []byte("new")) }()
	require.NoError(t, waitPromotionResult(t, put, "foreground Put"))
